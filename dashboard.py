        ...
        if len(user_date_range) == 2:
            ...
            if not user_df.empty:
                ...
                fig_text_length = px.bar(
                    df_text_length, y='user', x=['user_text_length', 'ai_text_length'],
                    orientation='h', barmode='group',
                ...
                # ... existing code ...
                # 新增：每日文字量趋势图表（需求2）
                st.subheader("每日文字量趋势")

                # 按日期和用户分组，计算每日文字量
                daily_text_df = filtered_user_daily_df.groupby(['created_at', 'user_name']).agg({
                ...
                # 生成完整的日期范围
                all_dates = pd.date_range(start=start_date_user, end=end_date_user, freq='D')
                all_users = daily_text_df['user_name'].unique()
